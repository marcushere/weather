...
public class DetermineDeltaHigh implements Runnable {
	...
	public DetermineDeltaHigh(WeatherTerm wt, int threadID) {
		this.wt = wt;
		this.tID = threadID;
	}
	...
	public void run() {
		...
		try {
			...
			// delta_high for actual is the change from the previous day.
			// delta_high
			...
			for (int q = 0; q < 2; q++) {
				...
				while (rs.next()) {
					...
					if (i % 1000 == 0) {
						con.commit();
						if (!dhs.isEmpty()) wt.threadOutMessage(dhs, tID, 3);
						dhs = "";
						if (wt.isStop()) {
							rs.close();
							con.commit();
		...
		wt.threadOutMessage("delta high thread finished", tID, 3);
	}
